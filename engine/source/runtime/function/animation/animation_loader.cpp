...
namespace Movan
{
    namespace
    {
        ...
        std::shared_ptr<RawBone> createBone(SkeletonData* skeleton_data, int parent_index)
        {
            ...
            std::shared_ptr<RawBone> bone_ptr = std::make_shared<Movan::RawBone>();
            *bone_ptr                         = raw_bone;
    ...
    std::shared_ptr<Movan::AnimationClip> AnimationLoader::loadAnimationClipData(std::string animation_clip_url)
    {
        AnimationAsset animation_clip;
        g_runtime_global_context.m_asset_manager->loadAsset(animation_clip_url, animation_clip);
        return std::make_shared<Movan::AnimationClip>(animation_clip.clip_data);
    }

    std::shared_ptr<Movan::SkeletonData> AnimationLoader::loadSkeletonData(std::string skeleton_data_url)
    {
        SkeletonData data;
        g_runtime_global_context.m_asset_manager->loadAsset(skeleton_data_url, data);
        return std::make_shared<Movan::SkeletonData>(data);
    }

    std::shared_ptr<Movan::AnimSkelMap> AnimationLoader::loadAnimSkelMap(std::string anim_skel_map_url)
    {
        AnimSkelMap data;
        g_runtime_global_context.m_asset_manager->loadAsset(anim_skel_map_url, data);
        return std::make_shared<Movan::AnimSkelMap>(data);
    }

    std::shared_ptr<Movan::BoneBlendMask> AnimationLoader::loadSkeletonMask(std::string skeleton_mask_file_url)
    {
        BoneBlendMask data;
        g_runtime_global_context.m_asset_manager->loadAsset(skeleton_mask_file_url, data);
        return std::make_shared<Movan::BoneBlendMask>(data);
    }
