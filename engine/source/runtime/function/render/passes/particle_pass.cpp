...
namespace Movan
{
    ...
    void ParticlePass::setupAttachments()
    {
        ...
        // movan texture
        {
            std::shared_ptr<TextureData> m_movan_logo_texture_resource = m_render_resource->loadTexture(
                m_particle_manager->getGlobalParticleRes().m_movan_logo_texture_path, true);
            m_rhi->createGlobalImage(m_movan_logo_texture_image,
                                     m_movan_logo_texture_image_view,
                                     m_movan_logo_texture_vma_allocation,
                                     m_movan_logo_texture_resource->m_width,
                                     m_movan_logo_texture_resource->m_height,
                                     m_movan_logo_texture_resource->m_pixels,
                                     m_movan_logo_texture_resource->m_format);
        }
    ...
    void ParticlePass::setupDescriptorSetLayout()
    {
        ...
        // compute descriptor sets
        {
            ...
            {
                RHIDescriptorSetLayoutBinding& movan_texture_layout_binding = particle_layout_bindings[10];
                movan_texture_layout_binding.binding                        = 10;
                movan_texture_layout_binding.descriptorType  = RHI_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
                movan_texture_layout_binding.descriptorCount = 1;
                movan_texture_layout_binding.stageFlags      = RHI_SHADER_STAGE_COMPUTE_BIT;
            }
    ...
    void ParticlePass::updateDescriptorSet()
    {
        for (int eid = 0; eid < m_emitter_count; ++eid)
        {
            // compute part
            {
                ...
                RHIDescriptorImageInfo movan_texture_image_info = {};
                movan_texture_image_info.sampler                = sampler;
                movan_texture_image_info.imageView              = m_movan_logo_texture_image_view;
                movan_texture_image_info.imageLayout            = RHI_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

                {
                    ...
                    descriptorset.pImageInfo             = &movan_texture_image_info;
                    descriptorset.descriptorCount        = 1;
                }
